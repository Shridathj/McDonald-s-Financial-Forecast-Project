...
def cash_flow_statement():
    ...
    for df in [df_adjusted_full, df_unadjusted_full]:
        ...
        df.loc[df["Metrics"] == "Total Cash Provided by (Used for) Financing Activities", "2014":"2018"] = (
            financing_components.values
        )
        ...
        fcf_val = df.loc[df["Metrics"] == "Total Cash Provided by (Used for) Financing Activities", "2014":"2018"].iloc[
            0
        ]
        ...
        cash_balances_sale = df.loc[df["Metrics"] == "Cash in Cash Balances of Business for Sale", "2014":"2018"].iloc[
            0
        ]
        ...
        total_revenue = pd.Series(
            [29107.1, 26923.7, 25760.6, 23377.8, 21025.2], index=["2014", "2015", "2016", "2017", "2018"]
        )
        total_debt = pd.Series(
            [18038.7, 27741.7, 29228.2, 31427.4, 32171.6], index=["2014", "2015", "2016", "2017", "2018"]
        )
        net_income = pd.Series([5046.8, 4798.2, 4903.2, 5319.1, 5924.3], index=["2014", "2015", "2016", "2017", "2018"])
        total_assets = pd.Series(
            [36315.3, 40177.1, 32451.0, 34615.0, 32811.2], index=["2014", "2015", "2016", "2017", "2018"]
        )
        share_holderseq = pd.Series(
            [13636.4, 7505.1, -2306.5, -3346.8, -6258.4], index=["2014", "2015", "2016", "2017", "2018"]
        )
        market_cap = pd.Series(
            [91190.0, 108480.0, 101080.0, 137210.0, 136890.0], index=["2014", "2015", "2016", "2017", "2018"]
        )
    ...
    def apply_styles(df):
        ...
        styled_df = styled_df.format(
            format_currency, subset=pd.IndexSlice[df["Metrics"].isin(metrics_monetary + ocf_capex), df.columns[1:]]
        )

        # Apply blank
        styled_df = styled_df.format(
            format_blank, subset=pd.IndexSlice[df["Metrics"].isin(section_headers + subsection_headers), df.columns[1:]]
        )
        ...
        styled_df = styled_df.set_properties(
            **{"font-weight": "normal", "text-align": "right", "background-color": "#1A1A1A"},
        ...
        styled_df = styled_df.set_properties(
            **{"background-color": "#67c0ff"},
            subset=pd.IndexSlice[df["Metrics"].isin(ratio_metrics + ocf_capex), df.columns[1:]],
        )
        ...
        def highlight_negatives(val):
            ...
            if isinstance(val, (int, float)) and val < 0:
                ...
                is_ratio_metric = (
                    df.loc[df.index[df[df.columns[1:]].eq(val).any(axis=1)].tolist()[0], "Metrics"] in ratio_metrics
                )
        ...
        styled_df = styled_df.map(
            highlight_negatives,
            subset=pd.IndexSlice[~df["Metrics"].isin(subsection_headers + section_headers), df.columns[1:]],
        )
        ...
        styled_df = (
            ...
            .set_properties(
                **{
                ...
                subset=pd.IndexSlice[~df["Metrics"].isin(section_headers + subsection_headers), df.columns[1:]],
            )
            .set_table_styles(
                [
                    {
    ...
    st.markdown(
        "**CPI Adjustment Note:** Monetary values have been CPI-adjusted to reflect real purchasing power. This ensures consistency in economic analysis across years. Minor rounding differences (0.03–0.04%) may occur due to CPI multiplier precision, as noted in the Excel file, but these have negligible impact."
    )
