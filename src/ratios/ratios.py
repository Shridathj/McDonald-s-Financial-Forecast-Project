...
def ratio_analysis():
    ...
    def style_table(df, title):
        ...
        if "OCF-CapEx" in df["Metrics"].values:
            styled = styled.format(
                format_percentage, subset=pd.IndexSlice[df["Metrics"] != "OCF-CapEx", df.columns[1:]]
            )
    ...
    def plot_return_ratios(df):
        ...
        fig.update_yaxes(
            title_text="OCF/Shareholder's equity (%)",
            secondary_y=True,
            side="right",
            tickformat=",.1f",
            range=[-300, 100],
    ...
    def plot_dividend_sustainability(df):
        ...
        fig.update_layout(
            title="Dividend and Sustainability",
            xaxis_title="Year",
            template="plotly_white",
            height=400,
            showlegend=True,
