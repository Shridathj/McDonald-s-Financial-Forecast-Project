...
def market_research():
    ...
    unadj_market_df = pd.DataFrame({"Year": years, "Global Fast Food Market ($M) (Unadjusted)": unadj_markets}).round(2)
    ...
    col_order = [
        "Year",
        "Global Fast Food Market ($M)",
        "McDonald's Total Revenue ($M)",
        "Revenue Growth (RG)",
    ...
    def style_table(df, title, adjustment="", caption=""):
        ...
        if "Revenue Growth (RG)" in df.columns:
            styled = df.style.format({**fmt_dict, "Revenue Growth (RG)": format_percentage}).set_caption(
                f"{title} {adjustment}<br><small>{caption}</small>"
            )
        else:
    ...
    st.markdown(
        f"**Adjusted to 2018 USD via BLS CPI-U multipliers. Average RG (Adjusted): {avg_rg_adj:.2%}. Cycle Phase: All Expansion.**"
    )
    st.markdown(
        f"Market Size Source: [IBISWorld Global Fast Food Restaurants](https://www.ibisworld.com/global/market-size/global-fast-food-restaurants/1480/)"
    )
