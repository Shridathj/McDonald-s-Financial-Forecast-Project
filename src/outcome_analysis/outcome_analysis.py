...
def outcome_analysis():
    ...
    styled_df = df.style.set_table_styles(
        [
            {
                "selector": 'th.col_heading[col="Actual Market"]',
                "props": [("background-color", "#e6f3ff"), ("color", "navy"), ("font-weight", "bold")],
            },
            {
                "selector": 'th.col_heading[col="Actual Revenue"]',
                "props": [("background-color", "#f0f8ff"), ("color", "black"), ("font-weight", "bold")],
            },
            {
                "selector": 'th.col_heading[col="Baseline Market"], th.col_heading[col="Pandemic Market"], th.col_heading[col="Hindsight Market"]',
                "props": [("background-color", "#f5f5f5"), ("color", "gray"), ("font-weight", "bold")],
            },
            {
                "selector": 'th.col_heading[col="Baseline Revenue"], th.col_heading[col="Pandemic Revenue"], th.col_heading[col="Hindsight Revenue"]',
                "props": [("background-color", "#fff2e6"), ("color", "darkorange"), ("font-weight", "bold")],
            },
            {
                "selector": "th",
                "props": [("font-weight", "bold"), ("text-align", "center"), ("border", "1px solid #ddd")],
            },
    ...
    err_years = years[5:]
    pand_err = np.round(
        ((np.array(pandemic_market[5:]) - np.array(actual_market[5:])) / np.array(actual_market[5:]) * 100), 2
    )
    hind_err = np.round(
        ((np.array(hindsight_market[5:]) - np.array(actual_market[5:])) / np.array(actual_market[5:]) * 100), 2
    )
    ...
    styled_err = (
        err_df.style.map(color_errors, subset=err_df.columns)
        .set_table_styles(
            [
                {
                    "selector": "th",
    ...
    fig.update_yaxes(
        title_text="Global Fast-food Market Size (Millions)",
        secondary_y=False,
