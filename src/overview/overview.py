...
def overview():
    ...
    def apply_styles(df):
        ...
        styled_df = styled_df.format(
            format_currency, subset=pd.IndexSlice[~df["Metric"].isin(ratio_metrics), df.columns[1:-1]]
        )

        # Apply percentage format to ratio metrics and CAGR
        styled_df = styled_df.format(
            format_percentage, subset=pd.IndexSlice[df["Metric"].isin(ratio_metrics), df.columns[1:]]
        )
        ...
        styled_df = styled_df.set_properties(
            **{"background-color": "#67c0ff"}, subset=pd.IndexSlice[df["Metric"].isin(ratio_metrics), :]
        )
    ...
    key_ratios = df_adjusted[
        df_adjusted["Metric"].isin(["Revenue Growth", "Operating Margin", "Net Margin", "EPS Growth"])
    ].copy()
    ...
    ratios_data = df_adjusted[
        df_adjusted["Metric"].isin(["Revenue Growth", "EPS Growth", "Operating Margin", "Net Margin"])
    ].copy()
    ratios_data = ratios_data.melt(
        id_vars=["Metric"], value_vars=["2014", "2015", "2016", "2017", "2018"], var_name="Year", value_name="Value"
    )
