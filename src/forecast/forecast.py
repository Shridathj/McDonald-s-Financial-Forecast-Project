...
"""
    ...
    def style_assumptions(df):
        ...
        styled = (
            df.style.format({"Value": assumption_formatter})
            .set_properties(
                **{
                    "text-align": "center",
    ...
    def style_table(df):
        ...
        styled = styled.set_properties(
            **{"font-weight": "bold", "text-align": "center", "background-color": "#2D2D2D", "color": "white"},
            subset=pd.IndexSlice[:, data_cols],
        )

        styled = styled.set_properties(
            **{"text-align": "left", "font-weight": "bold", "background-color": "#1A1A1A", "color": "white"},
    ...
    fig1.add_trace(
        go.Scatter(
            x=years,
            y=adjusted_revenue_baseline,
            name="Adjusted Revenue (Millions USD)",
            mode="lines+markers",
    ...
    fig2.add_trace(
        go.Scatter(
            x=years,
            y=proj_market_pandemic_B,
            name="Projected Market Size (1.5% CAGR)",
            mode="lines+markers",
            line=dict(width=2, color=global_colors[1]),
            marker=dict(size=6),
    ...
    fig2.add_trace(
        go.Scatter(
            x=years,
            y=hindsight_market_pandemic_B,
            name="Hindsight Market Size (2% CAGR)",
            mode="lines+markers",
