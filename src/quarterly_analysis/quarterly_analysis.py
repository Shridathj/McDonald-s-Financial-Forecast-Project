...
def quarterly_analysis():
    ...
    def compute_volatility_metrics(revenues_df):
        ...
        volatility = pd.DataFrame(
            {"Year": revenues_df["Year"], "Mean": [None] * n, "Stnd Dev": [None] * n, "Volatility": [None] * n}
        )
    ...
    st.write(
        style_table(
            seasonality,
            "Seasonality Factors",
            "Computed as (Quarterly Revenue / (Annual Total / 4)); Yearly Factor = Avg of Q1-Q4 Seasonalites",
        )
    )
    st.write(
        style_table(
            volatility,
            "Volatility Metrics",
            "Computed: Mean/Std Dev of Quarters per Year; Volatility = Std Dev / Mean (All in $M except %; based on unadjusted)",
        )
    )
    ...
    def plot_seasonality_heatmap():
        fig = go.Figure(
            data=go.Heatmap(
                z=[
                    [
                        seasonality.loc[i, "Q1"],
                        seasonality.loc[i, "Q2"],
                        seasonality.loc[i, "Q3"],
                        seasonality.loc[i, "Q4"],
                    ]
                ...
                colorscale="Viridis",
                text=[
                    [f"{seasonality.loc[i, q]:.4f}" for q in ["Q1", "Q2", "Q3", "Q4"]] for i in range(len(seasonality))
                ],
                texttemplate="%{text}",
                textfont={"size": 10},
                hoverongaps=False,
        ...
        fig.update_layout(
            title="Seasonality Factors Heatmap",
            xaxis_title="Quarter",
            yaxis_title="Year",
