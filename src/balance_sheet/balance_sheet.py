...
def balance_sheet():
    ...
    for metric in df_unadjusted["Metrics"]:
        if metric not in ratio_metrics and metric not in [
            "ASSETS",
            "Current Assets",
            "Other Assets",
            "Property and Equipment",
    ...
    for df in [df_adjusted, df_unadjusted]:
        ...
        # Total Other Assets
        other_assets = df.loc[
            df["Metrics"].isin(["Investments in and Advances to Affiliates", "Goodwill", "Miscellaneous"]),
            "2014":"2018",
        ].sum()
        ...
        # Net Property and Equipment
        net_property = df.loc[
            df["Metrics"].isin(["Property and Equipment, at Cost", "Accumulated Depreciation and Amortization"]),
            "2014":"2018",
        ].sum()
        ...
        # Total Assets
        total_assets = df.loc[
            df["Metrics"].isin(["Total Current Assets", "Total Other Assets", "Net Property and Equipment"]),
            "2014":"2018",
        ].sum()
        df.loc[df["Metrics"] == "Total Assets", "2014":"2018"] = total_assets.values

        # Total Current Liabilities
        current_liabilities = df.loc[
            df["Metrics"].isin(
                [
                    "Accounts Payable",
                    "Income Taxes",
        ...
        # Total Liabilities and Shareholder's Equity
        total_liab_equity = df.loc[
            df["Metrics"].isin(
                ["Total Current Liabilities", "Total Long-term Liabilities", "Total Shareholder's Equity (Deficit)"]
            ),
            "2014":"2018",
        ].sum()
    ...
    def apply_styles(df):
        ...
        styled_df = styled_df.format(
            format_currency, subset=pd.IndexSlice[~df["Metrics"].isin(ratio_metrics), df.columns[1:]]
        )

        # Apply percentage format to ratio metrics
        styled_df = styled_df.format(
            format_percentage, subset=pd.IndexSlice[df["Metrics"].isin(ratio_metrics), df.columns[1:]]
        )

        # Apply bold and centered text to Metrics headers
        styled_df = styled_df.set_properties(
            **{"font-weight": "bold", "text-align": "center", "background-color": "#F85B72"},
            subset=pd.IndexSlice[df["Metrics"].isin(Metrics_headers1), :],
        )

        styled_df = styled_df.set_properties(
            **{"font-weight": "bold", "text-align": "left", "background-color": "#d3d3d3"},
            subset=pd.IndexSlice[df["Metrics"].isin(Metrics_headers2), :],
        )

        styled_df = styled_df.set_properties(
            **{"font-weight": "normal", "text-align": "left", "background-color": "#1A1A1A"},
            subset=pd.IndexSlice[df["Metrics"].isin(subheaders), :],
        )

        # Apply blue highlighting to ratio rows
        styled_df = styled_df.set_properties(
            **{"background-color": "#67c0ff"}, subset=pd.IndexSlice[df["Metrics"].isin(ratio_metrics), :]
        )
        ...
        styled_df = styled_df.map(
            highlight_negatives, subset=pd.IndexSlice[~df["Metrics"].isin(Metrics_headers2), df.columns[1:]]
        )
        ...
        styled_df = styled_df.set_properties(
            **{
            ...
            subset=pd.IndexSlice[~df["Metrics"].isin(Metrics_headers1), df.columns[1:]],
        ).set_table_styles(
            [
                {
                    "selector": "th",
    ...
    styled_cpi = (
        df_cpi.style.set_caption("CPI Data")
        .format(na_rep="")
        .set_properties(
            **{
                "text-align": "left",
    ...
    key_ratios = df_adjusted[
        df_adjusted["Metrics"].isin(
            ["Key Ratios", "Return on Assets (ROA)", "Debt to Asset Ratio (D/A)", "Current Ratio"]
        )
    ].copy()
    ...
    st.markdown(
        "**CPI Adjustment Note:** Monetary values have been CPI-adjusted to reflect real purchasing power, while common stock remains unadjusted as it represents share count, not monetary value. This may cause a minor variance in the balance sheet equation due to mixed adjustment methodologies."
    )
    ...
    st.markdown(
        "[CPI Data Source: BLS CPI Data](https://www.bls.gov/regions/mid-atlantic/data/consumerpriceindexhistorical_us_table.htm)"
    )

    # Plot Key Ratios (Bar Chart)
    ratios_data = df_adjusted[
        df_adjusted["Metrics"].isin(["Return on Assets (ROA)", "Debt to Asset Ratio (D/A)", "Current Ratio"])
    ].copy()
    ratios_data = ratios_data.melt(
        id_vars=["Metrics"], value_vars=["2014", "2015", "2016", "2017", "2018"], var_name="Year", value_name="Value"
    )
    ...
    if px is not None:
        ...
        fig.update_layout(
            ...
            xaxis=dict(
                tickmode="array",
                tickvals=["2014", "2015", "2016", "2017", "2018"],
                ticktext=["2014", "2015", "2016", "2017", "2018"],
            ),
